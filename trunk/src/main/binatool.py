...
#		factory.add('fullscreen', iconset)
		...
		actions = (
			...
			('CloseAll', gtk.STOCK_CLOSE, _('_Close All Windows'), 'Right', _('Close All Windows'), self.close_all_windows)
			)
		toggle_actions = (
...
#		icon_path = self.find_path('mirage.png')
		...
		# add algorithm
#		self.tcombo = gtk.combo_box_entry_new_text() #combo_box_entry_new_text()
#		
#		self.tcombo.append_text(rotsuI_label)
#		self.tcombo.append_text(rotsuII_label)
#		self.tcombo.append_text(rotsuIII_label)
#		self.tcombo.append_text(rotsuTest_label)
#		self.tcombo.append_text(otsu_label)
#		self.tcombo.append_text(niblack_label)
#		self.tcombo.append_text(sauvola_label)
#		self.tcombo.append_text(met_label)
#		self.tcombo.child.connect('changed', self.changed_cb)
#		self.tcombo.set_active(0)
#		
#		self.runbutton = gtk.Button()
#		self.runbutton.set_label("      Run      ")
#		self.runbutton.connect("clicked", self.runbtn_clicked)
#		
#		hboxThresh = gtk.HBox(False, 0)
#		hboxThresh.pack_start(self.tcombo, False, False, 0) 
#		hboxThresh.pack_start(self.runbutton, False, False, 0)  
#
#		vbox.pack_start(hboxThresh, False, False, 0)
...
#		self.UIManager.get_widget('/MainMenu/MiscKeysMenuHidden').set_property('visible', False)
	...
	def runbtn_clicked(self,action):#This is when the user clicks the run button
		...
		print filename
		self.listAlg[self.curr_algorithm](filename)
...
#		self.UIManager.get_widget('/MainMenu/MiscKeysMenuHidden').set_property('visible', False)
	...
	def thumbnail_get_name(self, image_name):
		...
		m.update(uriname)
		mhex = m.hexdigest()
		mhex_filename = os.path.expanduser('~\\.thumbnails\\normal\\' + mhex + '.png')
	...
	def thumbpane_get_pixbuf(self, thumb_url, image_url, force_generation):
		...
		try:
			...
			pix.save(thumb_url, "png", {'tEXt::Thumb::URI':uri, 'tEXt::Thumb::MTime':file_mtime, 'tEXt::Software':'Mirage'})
			return pix
		except:
			return None
	...
	def parse_action_command2(self, cmd, imagename):
		...
		if shell_rc != 0:
			msg = _('Unable to launch') \
			    + ' \"' + cmd + '\". ' \
			    + _('Please specify a valid command from Edit > Custom Actions.')
...
#			gtk_recent_manager = gtk.recent_manager_get_default()
			gtk_recent_manager = gtk.RecentManager.get_default()
			uri = ''
			if imgfile[:7] != 'file://':
				uri = 'file://'
